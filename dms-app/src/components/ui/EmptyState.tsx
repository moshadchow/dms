...
export default function EmptyState({ icon = DEFAULT_ICON, title, description, action, compact }: Props) {
  return (
    <div style={{
      ...
      <p style={{ fontWeight: 600, color: 'var(--text-tertiary)', margin: 0, fontSize: compact ? '0.85rem' : '0.95rem' }}>
        {title}
      </p>
      {description && (
        <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', margin: '6px 0 0', lineHeight: 1.6, maxWidth: '280px' }}>
          {description}
        </p>
      )}
